document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupMusicControl() {
        ...
        // Try to play music immediately in multiple ways
        playAudio();
        
        // Force play using load() then play()
        backgroundMusic.load();
        backgroundMusic.play().catch(e => console.log("Initial play failed:", e));
        ...
        // Try to play on any user interaction - this is the most reliable method
        const userInteractionEvents = ['click', 'touchstart', 'keydown', 'mousemove', 'scroll'];
        userInteractionEvents.forEach(event => {
            document.addEventListener(event, function playOnInteraction() {
                ...
                // Remove this specific event listener after first interaction
        ...
        // Try again after delays (some browsers need this)
        setTimeout(() => {
            backgroundMusic.muted = false;
            playAudio();
        }, 500);
    ...
    // Helper function to play audio and handle the button state
    function playAudio() {
    ...
    // Also try playing when window loads (all resources loaded)
